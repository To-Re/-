...
func GetExamKlassListByExamIds(exam_ids []int32) ([]*model.KlassExam, error) {
	db := dal.db
	list := []*model.KlassExam{}
	if err := db.Table(dal.klassExamTableName).Where("exam_id in (?)", exam_ids).Find(&list).Error; err != nil {
		return nil, err
	}
	return list, nil
}
